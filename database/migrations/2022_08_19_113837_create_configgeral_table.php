...
return new class extends Migration
{
    ...
    public function up()
    {
        Schema::connection('pdv')->create('configgeral', function (Blueprint $table) {
            ...
            $table->integer('flgTouch')->default(1);
            $table->integer('flgSangriaCartao')->default(0);
            $table->integer('flgSangriaFiado')->default(0);
            $table->decimal('limiteDiferenca', 19, 4)->default(0);
            $table->decimal('limiteDesconto', 6, 3)->default(0);
            $table->integer('flgcaixacartaoobrigatoriofiscal')->default(0);
            $table->integer('flgCodigoObrigatorio')->default(0);
            $table->integer('TipoRelFechamento')->default(-1);
            $table->integer('ImpSangriaAuto')->default(1);
            $table->integer('TamanhoCodEtiqueta')->default(4);
            $table->integer('flgCancelaSenha')->default(0);
            $table->integer('flgCancelaJustificativa')->default(0);
            $table->integer('flgCarregaPlaca')->default(0);
            $table->integer('flgImprimeDiferencaCaixa')->default(1);
            $table->string('PreFixoPreVenda')->nullable();
            $table->string('PreFixoComanda')->nullable();
            $table->integer('BoxDefault')->default(1);
            $table->integer('idConfigGeral')->default(1);
            $table->binary('imgBanner')->nullable();
            $table->integer('flgTrocoCartao')->default(0);
            $table->integer('flgSenhaCancelamentoCupom')->default(1);
            $table->timestamps();
        });
    }
